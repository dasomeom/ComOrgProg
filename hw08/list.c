...
static struct list_node* create_node(struct person *data)
{
    ...
    if ((temp = (struct list_node*)malloc(sizeof(struct list_node))) == NULL) {
...
struct list *copy_list(struct list *listToCopy) {
    ...
    struct list *aList;
    if ((aList = create_list()) == NULL) {
        person_free(aList->head->data);
        free(aList);
        return NULL;
    }

    if (listToCopy == NULL || aList == NULL ||listToCopy->size == 0 ) { free(aList); return NULL;}
    struct list_node *tmp = listToCopy->head;
    struct person *temp;
    int a = 1;
    while (tmp != NULL && a == 1) {
        person_copy(tmp->data, &temp);
        a = push_back (aList, temp);
        tmp = tmp->next;
    }
    if (a == 0) {
        person_free(temp);
...
{
    if (listToPrependTo == NULL) {return 0;}

    struct list_node *aNode = create_node(data);
    if (aNode == NULL) { return 0;}
    if (listToPrependTo->size == 0) {
        listToPrependTo->head = aNode;
        listToPrependTo->tail = aNode;
        listToPrependTo->size++;
        return 1;
    } else {
        aNode->next = listToPrependTo->head;
        listToPrependTo->head = aNode;
        listToPrependTo->size++;
        return 1;
    }
...
{
    if (listToAppendTo == NULL) {return 0;}
    struct list_node *aNode = create_node(data);
    if (aNode == NULL) { return 0;}
    if (listToAppendTo->size == 0) {
        listToAppendTo->head = aNode;
        listToAppendTo->tail = aNode;
        listToAppendTo->size++;
        return 1;
    }
    struct list_node *aTail = listToAppendTo->tail;
    aTail->next = aNode;
    listToAppendTo->tail = aNode;
    listToAppendTo->size++;
    return 1;
...
int add(struct list *listToAddTo, int index, struct person *data)
{
    if (listToAddTo == NULL|| index < 0 || index > listToAddTo->size) {
        ...
        struct list_node *aNode = create_node(data);
        listToAddTo->head = aNode;
        listToAddTo->tail = aNode;
        listToAddTo->size++;
        return 1;
    } else if (index == listToAddTo->size) {
        struct list_node *aNode = create_node(data);
        listToAddTo->tail->next = aNode;
        listToAddTo->tail = listToAddTo->tail->next;
        listToAddTo->size++;
        return 1;
    } else {
        struct list_node *aNode = create_node(data);
        struct list_node *tmp = listToAddTo->head;
...
int front(struct list *listToGetFrontOf, struct person **dataOut) {
    //(struct list*)malloc(sizeof(listToGetFrontOf)) == NULL
    if (listToGetFrontOf == NULL || dataOut == NULL || listToGetFrontOf->size == 0 ) {
...
{
    if (listToGetBackOf == NULL || dataOut == NULL || listToGetBackOf->size == 0 ) {
        if (dataOut!=NULL){*dataOut = NULL;}
        return 0;
    } else {
        struct list_node *aPointer = listToGetBackOf->tail;
        if (aPointer->data != NULL) {
            *dataOut = aPointer->data;

            return 1;
        } else {
            *dataOut = NULL;
            person_free(aPointer->data);
            return 0;
        }
...
{
    if (listToGetDataFrom == NULL|| index < 0 || index>=listToGetDataFrom->size || dataOut == NULL) {
        if (dataOut != NULL) {*dataOut = NULL;}
        return 0;
    } else if (index == 0) {
        struct list_node *aPointer = listToGetDataFrom->head;
        *dataOut = aPointer->data;
        return 1;
    } else if (index == listToGetDataFrom->size-1) {
        struct list_node *aPointer = listToGetDataFrom->tail;
        *dataOut = aPointer->data;
        return 1;
    } else {
        struct list_node *tmp = listToGetDataFrom->head;
        for (int i = 0; i < index; i++) {
            tmp = tmp->next;
        }
        struct list_node *aPointer = tmp;
        *dataOut = aPointer->data;
        return 1;
    }
}
...
{
    if (listToSearch == NULL|| listToSearch->size == 0 || dataOut == NULL) {
        if (dataOut != NULL) {*dataOut = NULL;} return 0;
    }
    struct list_node *temp = listToSearch->head;
    while (temp!= NULL) {
        if (person_eq_name(temp->data, data) == 0) {
            *dataOut = temp->data;
            return 1;}
        else {temp = temp->next;}
    }
...
{
    if (listToPopFrom == NULL || dataOut == NULL){
        if (dataOut != NULL){*dataOut = NULL;}
        return 0;
    } else if(listToPopFrom->size == 0 || dataOut == NULL) {
        if (dataOut != NULL){*dataOut = NULL;}
        return 0;
    } else if (listToPopFrom->size == 1) {
        struct list_node *aPointer = listToPopFrom->head;
        int a = person_copy(aPointer->data, dataOut);
        empty_list(listToPopFrom);
        return a;
    } else {
        struct list_node *aPointer = listToPopFrom->head;
        person_copy(listToPopFrom->head->data, dataOut);
...
{
    if (listToPopFrom == NULL || dataOut == NULL || listToPopFrom->size == 0) {
        if (dataOut != NULL){*dataOut = NULL;}
        return 0;
    } else if (listToPopFrom->size == 1) {
        person_copy(listToPopFrom->head->data,dataOut);
        //person_free(listToPopFrom->head->data);
        empty_list(listToPopFrom);
        return 1;
    } else if (listToPopFrom->size == 2) {
        person_copy(listToPopFrom->tail->data,dataOut);
        person_free(listToPopFrom->tail->data);
        free(listToPopFrom->tail);
        listToPopFrom->head->next = NULL;
        listToPopFrom->tail = listToPopFrom->head;
        listToPopFrom->size = 1;
        return 1;
    } else {
        person_copy(listToPopFrom->tail->data, dataOut);
        struct list_node *aNode = listToPopFrom->head;
        while (aNode->next->next != NULL) {
            aNode = aNode->next;
        }
        person_free(aNode->next->data);
        free(aNode->next);
        listToPopFrom->tail = aNode;
        aNode->next = NULL;
        listToPopFrom->size = listToPopFrom->size - 1;
        return 1;
    }
...
{
    if (listToRemoveFrom == NULL || dataOut == NULL || listToRemoveFrom->size <= 0 ||data_to_remove== NULL) {
        if (dataOut != NULL) {*dataOut = NULL;}
        return 0;
    }
    if (listToRemoveFrom->size == 1 && person_eq_name(listToRemoveFrom->head->data,data_to_remove)==0) {
        person_copy(listToRemoveFrom->head->data,dataOut);
        person_free(listToRemoveFrom->head->data);
        free(listToRemoveFrom->head);
        listToRemoveFrom->head = NULL;
        listToRemoveFrom->tail = NULL;
        listToRemoveFrom->size = 0;
        return 1;
    } else if (person_eq_name(listToRemoveFrom->head->data,data_to_remove)==0 && listToRemoveFrom->size > 1) {
        person_copy(listToRemoveFrom->head->data,dataOut);
        struct list_node *temp = listToRemoveFrom->head;
        listToRemoveFrom->head = temp->next;
        person_free(temp->data);
        free(temp);
        listToRemoveFrom->size = listToRemoveFrom->size - 1;
        return 1;
    } else if (person_eq_name(listToRemoveFrom->tail->data,data_to_remove)==0 && listToRemoveFrom->size >= 2) {
        person_copy(listToRemoveFrom->tail->data,dataOut);
        struct list_node *temp = listToRemoveFrom->head;
        while(temp->next->next != NULL) {temp = temp->next;}
        listToRemoveFrom->tail = temp;
        temp = temp->next;
        person_free(temp->data);
        free(temp);
        ...
        int b = 100;
        struct list_node *temp = listToRemoveFrom->head;
        while(b != 0 && temp->next != NULL) {
            b = person_eq_name(temp->next->data,data_to_remove);
            if (b != 0) {temp = temp->next;}
        }
        if (b != 0 && temp->next == NULL) {*dataOut = NULL; return 0;}
        if (b == 0) {
            person_copy(temp->next->data, dataOut);
            if (temp->next->next != NULL) {
                struct list_node * temtem = temp->next;
                temp->next = temp->next->next;
                person_free(temtem->data);
                free(temtem);
            } else {
                person_free(temp->next->data);
                free(temp->next);
                temp->next = NULL;
            }
            listToRemoveFrom->size = listToRemoveFrom->size - 1;
...
{
    if (listToEmpty != NULL) {
        struct list_node *temp = listToEmpty->head;
        while(listToEmpty->size > 0) {
            struct list_node *aNode = temp;
            temp = temp->next;
...
{
    if (listToReverse == NULL ) {return 0;}
    if (listToReverse->size == 0 || listToReverse->size == 1) { return 1;}
    struct list_node *temp = listToReverse->head;

    while (temp->next->next != NULL) {
        struct list_node *per = temp->next;
        temp->next = per->next;
        per->next = listToReverse->head;
        listToReverse->head = per;

    }
    struct list_node *headToBe= temp->next;
    headToBe->next = listToReverse->head;
    listToReverse->head = headToBe;
    temp->next = NULL;
...
{
    if (firstList == NULL || lastList == NULL) {return 0;}
    if (firstList->size == 0 && lastList->size != 0) {
        ...
        lastList->size = 0;
        return 1;
    } else if (firstList->size != 0 && lastList->size == 0) {
        lastList->head = NULL;
        lastList->tail = NULL;
        lastList->size = 0;
        return 1;
